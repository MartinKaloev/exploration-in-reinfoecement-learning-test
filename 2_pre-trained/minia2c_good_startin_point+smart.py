...
def create_actor2(state_shape, action_shape):
    ...
    model.add(keras.layers.Dense(42, activation=tf.keras.layers.LeakyReLU(), kernel_initializer=init))
    model.add(keras.layers.Dense(action_shape, activation='softmax', kernel_initializer=init))
    model.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.Adam(lr=learning_rate), metrics=['accuracy'])
    return model
...
def main():
    ...
    actor_checkpoint_path2 = "training_actor2/actor_cp.ckpt"
    critic_checkpoint_path2 = "training_critic2/critic_cp.ckpt"
    ...
    if os.path.exists('training_actor'):
        ...
        actor2.load_weights(actor_checkpoint_path2)

        critic2.load_weights(critic_checkpoint_path2)
    #print(actor)
    #print(critic)
