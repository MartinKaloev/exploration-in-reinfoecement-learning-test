...
def create_critic2(state_shape, output_shape):
    learning_rate = 0.001
    init = tf.keras.initializers.HeUniform()
    model = keras.Sequential()
    model.add(keras.layers.Dense(124, input_shape=state_shape, activation=tf.keras.layers.LeakyReLU(), kernel_initializer=init))
...
def main():
    ...
    critic_checkpoint_path_ = "training_critic_/critic_cp.ckpt"
    actor_checkpoint_path2_ = "training_actor_/actor_cp.hdf5"
    critic_checkpoint_path2_ = "training_critic_/critic_cp.hdf5"
    ...
    critic2 = create_critic2(env.observation_space.shape, 1)
    critic2.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=tf.keras.optimizers.Adam(lr=0.001), metrics=['accuracy'])
    ...
    for episode in range(train_episodes):
        ...
        while not done:
            ...
            action_probs = actor2.predict(observation_reshaped).flatten()
            #action_probs2 = actor.predict(observation_reshaped)
            ...
            if (episode <1060):
                value_curr = np.asscalar(np.array(critic2.predict(observation_reshaped)))
                value_next = np.asscalar(np.array(critic2.predict(next_observation_reshaped)))
            ...
            #print(np.around(action_probs, 2), np.around(value_next - value_curr, 3), 'Advantage:', np.around(advantage, 2))
                ...
                critic2.train_on_batch(observation_reshaped, TD_target)
            #critic.fit(observation_reshaped, TD_target, verbose=0)
                ...
                actor2.train_on_batch(observation_reshaped, gradient_with_advantage)
            ...
            if done:
                ...
                print ( "ep:",episode ,"act num:", action_c  ,"r:", np.round(reward, 2), "big r: ",total_training_rewards," adv: ",np.round(advantage, 2) ,"act:", np.round(action,2) )
            ...
            if total_training_rewards > 100:
                actor2.save_weights(actor_checkpoint_path_)
                critic2.save_weights(critic_checkpoint_path_)
                actor2.save_weights(actor_checkpoint_path2_)
                critic2.save_weights(critic_checkpoint_path2_)
