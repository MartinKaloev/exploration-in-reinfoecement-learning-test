    ...
    for episode in range(train_episodes):
        ...
        while not done:
            ...
            if episode >60:
                ...
                #action = np.random.choice(env.action_space.n, 1, p=action_probs)[0]
                action=(np.argmax(action_probs))
                if (np.around(observation, 2)[2] > 0.12):
                    action=1
            #action=0
            #action =(np.argmax(action_probs))
            print("action:", action, "action_probs: ", action_probs, "states: ", observation)
            ...
            #critic.fit(observation_reshaped, TD_target, verbose=0)
                ...
                gradient_with_advantage = .001 * gradient * advantage_reshaped + action_probs
                #print("action: ", action, "probs: ", action_probs, "print encoded: ", encoded_action, "updated act: ", gradient_with_advantage)
